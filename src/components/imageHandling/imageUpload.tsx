					...
					<div className="w-full">
						...
						<div className="grid grid-cols-4 gap-3">
							{Array.from({ length: secondaryPhotoSlots }, (_, idx) => idx + 1).map((slotIndex) => {
								...
								const isUploadingThisSlot = isCompressing && photos.length === slotIndex;

								if (photo) {
									return (
										<div key={slotIndex} className="relative group aspect-3/4 rounded-xl overflow-hidden bg-gray-100 shadow-sm border border-gray-200">
								...
								if (slotIndex === photos.length && !primaryPhoto) {
									return (
										<div key={slotIndex} className="aspect-3/4 rounded-xl border border-dashed border-gray-200 bg-gray-50 flex items-center justify-center opacity-50 cursor-not-allowed">
								...
								return (
									<button
										key={slotIndex}
										type="button"
										onClick={slotIndex === photos.length ? onImageClick : undefined}
										disabled={isCompressing && photos.length !== slotIndex}
										className={cn(
											"aspect-3/4 rounded-xl flex items-center justify-center transition-all overflow-hidden relative",
											slotIndex === photos.length
										...
										{isUploadingThisSlot ? (
											...
											<div
												className={cn(
													...
													slotIndex === photos.length ? "bg-orange-50 group-hover:scale-110 group-hover:bg-orange-100 text-primary" : "text-gray-300"
												)}
											>
