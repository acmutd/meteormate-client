...
export default function LoginPage() {
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => { // what this function is doing is that, its handling email changes
    const value = e.target.value; // gets a value from the handleEmail change
    setEmail(value); // and sets the email

    if (!value.endsWith('@utdallas.edu')) { // check if the email is corrct or not
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => { // function to handle the passwords
    setPassword(e.target.value); 
  };

  const handleLogin = () => { // handles the login 
  ...
  return (
    <LogoBox logoSrc="/images/MM_logo_V1.png" logoAlt="MeteorMate Logo">
      ...
      <div className="text-right mt-2">
        <button className="text-black text-sm hover:underline cursor-pointer">Forgot password?</button>
      </div>
    </LogoBox>

    // <div className="bg-white/25 backdrop-blur-md rounded-xl p-8 text-black text-center drop-shadow-[#575757] drop-shadow-xl">
    ...
    //   </div>
      ...
      //     </div>
      //     <div className="relative w-full">
      //       <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="sabsolute left-4 top-1/2 transform translate-y-7 translate-x-3 w-5 h-5 text-black">
      //         <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 0 1 3 3m3 0a6 6 0 0 1-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1 1 21.75 8.25Z" />
      //       </svg>
